...
المرحلة الثامنة — المخطط البياني للعلاقات (مخطط Mermaid ملوّن ومفصّل)
هام جدا: يجب إنشاء مخطط Mermaid صالح تقنيا داخل كتلة كود مميزة بـ \`\`\`mermaid حيث يمكن عرضه مباشرة.
يجب أن يحتوي المخطط على أكبر قدر ممكن من المعلومات والعلاقات والتفاصيل.

قواعد صارمة لمخطط Mermaid:
- استعمل graph TD فقط
- يمنع منعا باتا استعمال أي وسوم HTML مثل <b> أو <i> أو <br> أو <br/> داخل مخطط Mermaid
- استعمل أسماء عقد بسيطة بأحرف لاتينية فقط مثل P1, P2, S1, ACT1
- ضع النصوص العربية داخل أقواس مربعة مع علامات اقتباس مزدوجة مثل: P1["أحمد محمد"]
- تجنب الأحرف الخاصة مثل : / \\ | ( ) داخل تسميات العقد
- لا تستعمل subgraph مع نصوص تحتوي على أحرف خاصة

تصنيف العقد بالألوان (إلزامي): استعمل class لربط كل عقدة بنوعها:
- person: للأشخاص الطبيعيين (P1, P2...)
- company: للشركات والأشخاص المعنويين (C1, C2...)
- vehicle: لوسائل النقل (V1, V2...)
- goods: للبضائع والمحجوزات (G1, G2...)
- location: للأماكن (L1, L2...)
- document: للوثائق والمراجع (D1, D2...)
- violation: للمخالفات (VIO1, VIO2...)
- officer: للأعوان والإطارات (O1, O2...)

أضف تفاصيل مهمة في تسميات العقد مثل المعرّفات والتواريخ والقيم.
استعمل نصوص وصفية على الأسهم لتوضيح نوع العلاقة.

مثال صحيح:
graph TD
P1["احمد محمد - بطاقة 12345"] -->|مسير| C1["شركة الأمل للتجارة"]
P1 -->|سائق| V1(["شاحنة - تونس 1234"])
C1 -->|مستورد| G1["قماش 500 كغ - قيمة 50000 د"]
L1{{"ميناء رادس"}} -->|مكان الضبط| G1
O1["العون أحمد - رقم 789"] -->|محرر المحضر| D1[/"محضر عدد 2024-123"/]
VIO1{"تهريب - فصل 214"} -->|منسوب لـ| P1
class P1 person
class C1 company
class V1 vehicle
class G1 goods
